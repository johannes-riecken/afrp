...
-- We really aught to test this, but we'll just settle for type checking
-- for now, since we know the gun works...
simpleGunObject :: Object
simpleGunObject = undefined

-- Since this file doesn't import ObjectBehavior, I'm going to place a
-- couple of versions of the code for ObjectBehavior.gun here.  These
-- correspond exactly with what is presented in the paper; they are
-- copied here just to ensure we don't make any obvious mistakes
-- that could be caught by parsing / typechecking.

-- *sigh*.  So much for typechecking...we have to comment this out anyway,
-- because oosGun is undefined.
{-
gun :: Position2 -> Object
gun (Point2 x0 y0) = proc objIn -> do
  let gi = oiGameInput objIn
  -- Desired position:
  (Point2 xd _) <- ptrPos -< gi
  rec
    -- Controller.
